...
6. Используя меню "File Upload" загрузить файл [ESPUtils.h](ESPHome/lib/ESPUtils.h)
...
8. Используя меню "File Upload" загрузить файлы внешнего компонента:
   - [__init__.py](esphome/components/espaudio/__init__.py)
   - [espaudio.h](esphome/components/espaudio/espaudio.h)
   - [espaudio.cpp](esphome/components/espaudio/espaudio.cpp)
...
13. Если прошивается впервые, то после успешной прошивки, и выхода устройства в Онлайн, нужно загрузить аудио файлы, для этого подключаемся к плате по FTP (пассивный режим, анонимное соединение без пароля и без TLS), и копируем файлы *.mp3 из папки [data](data/) в корень FTP
14. По желанию добавить карточку устройства на панель lovelace из файла [card.yaml](ESPHome/card.yaml) проекта. ![](images/card_2.png)
...
Электрическая схема и печатная плата версии 1.1 для самостоятельного производства (толщина текстолита используется 1.2мм - иначе в корпусе будет щель):
